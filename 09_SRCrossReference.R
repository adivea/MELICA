...
# replacing strings to avoid any inconsistencies in spelling across sources
...
# Adding NA rows to the shorter dataframe
if (row_diff > 0) {
  # unmatched_addressSR is longer
  extra_rows <- data.frame(bbr_leftovers = rep(NA, row_diff))
  unmatched_bbr <- bind_rows(unmatched_bbr, extra_rows)
} else {
  # unmatched_bbr is longer
  extra_rows <- data.frame(addressSR_leftovers = rep(NA, abs(row_diff)))
